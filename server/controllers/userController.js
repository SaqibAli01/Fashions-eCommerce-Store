...
export const userLogin = async (req, res) => {
    try {
        ...
        if (!user.verified) {
            return res.status(404).json({ message: "Account not verified. Please verify your email first." });
        }
...
export const updateUserInfo = async (req, res) => {
    try {
        ...
        if (user.email !== email) {
            ...
            await sendVerificationCode(email, verificationCode);
            // res.status(200).send({ message: 'User Info updated successfully Please verify your Email.' });
        } else {
            // If email is not being updated, set newEmail, verificationCode, and verificationCodeExpiresAt to null
            user.newEmail = null;
